  ...
  try {
    ...
    const offerCatalogSchema = {
      ...
      itemListElement: properties.map((prop, index) => {
        const [city, state] = prop.location?.split(",").map((part) => part.trim()) ?? [];
        const isAvailable =
          typeof prop.status === "string" ? prop.status.toLowerCase().includes("disponible") : true;
        const availability = isAvailable ? "https://schema.org/InStock" : "https://schema.org/OutOfStock";
        const price = typeof prop.price === "number" && Number.isFinite(prop.price) ? prop.price.toString() : "0";
        ...
        return {
          ...
          itemOffered: {
            "@type": "Residence",
            name: prop.title ?? "Propiedad destacada en Villanueva García",
    ...
    console.error("Error building dynamic offer catalog schema", caughtError);
    return null;
  }
}
